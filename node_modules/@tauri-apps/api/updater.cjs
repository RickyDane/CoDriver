...
async function onUpdaterEvent(handler) {
    return event.listen(event.TauriEvent.STATUS_UPDATE, (data) => {
...
async function installUpdate() {
    ...
    return new Promise((resolve, reject) => {
        ...
        // handled by rust
        event.emit(event.TauriEvent.INSTALL_UPDATE).catch((e) => {
            cleanListener();
            // dispatch the error to our checkUpdate
            throw e;
        });
...
async function checkUpdate() {
    ...
    return new Promise((resolve, reject) => {
        ...
        event.once(event.TauriEvent.UPDATE_AVAILABLE, (data) => {
        ...
        // start the process
        event.emit(event.TauriEvent.CHECK_UPDATE).catch((e) => {
            cleanListener();
            // dispatch the error to our checkUpdate
            throw e;
        });
...
exports.onUpdaterEvent = onUpdaterEvent;
